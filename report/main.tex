...
\begin{itemize}
    ...
    Foi optado por seguir com o modelo padrão linear de \textit{activation}, "\textit{relu}", pois a função linear de ativação é adequada para problemas de regressão \ref{} %https://www.turing.com/kb/how-to-choose-an-activation-function-for-deep-learning#choosing-the-right-activation-function-for-a-neural-network
...
Os testes reforçaram os achados iniciais, onde a melhor configuração teve como parâmetros \textit{ccp\_alpha} 0 e \textit{n\_estimators} 400.
...
Pela quantidade pequena de dados no conjunto de dados, foi escolhida fazer uma divisão dos dados utilizando 5-folds para determinação dos melhores hiperparâmetros, para então determinar o melhor desempenho global realizados pelos modelos de \textit{Redes-neurais}, \textit{Random Forest} e \textit{Regressão Linear} com os hiperparâmetros obtidos do capítulo \ref{sec: 5.1}.
